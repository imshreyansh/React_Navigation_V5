...
import { getAllWorkoutLevel, getByWorkoutCategory, addWorkout } from '../../../utils/api/addWorkoutAndDiet'
import { getMemberById } from '../../../utils/api/authorization'
import Loader from '../../../utils/resources/Loader'
import { showMessage, hideMessage } from "react-native-flash-message";
import boy from '../../../assets/images/boy.jpg'
import i18n from 'i18n-js'
class AddMemberWorkout extends Component {
    _isMounted = false
    ...
    onSubmit = () => {
        ...
        if (this.state.cardio === true && SelectedDates.length > 0) {
            ...
            this.state.SelectedDates.forEach(week => {
                memberWorkoutInfo.push({ member: this.props.navigation.getParam('memberId'), workoutCategories: 'Cardio', workoutsLevel: this.state.WorkoutCategory, dateOfWorkout: new Date(new Date(week.date)), workouts: works, note: this.state.note })
            })
            this.setState({
                loading: true
            })
        ...
        } else if (this.state.workouts === true && WorkoutCategorye !== null && SelectedDates.length > 0 && WorkoutCategorye === '' && WorkoutCategory !== '' && WorkoutCategory !== null) {
            const works = [], memberWorkoutInfo = []
            ...
            this.state.SelectedDates.forEach(week => {
                memberWorkoutInfo.push({ member: this.props.navigation.getParam('memberId'), workoutCategories: 'Workouts', workoutsLevel: this.state.WorkoutCategory, dateOfWorkout: new Date(new Date(week.date)), workouts: works, note: this.state.note })
            })
            this.setState({
                loading: true
            })
            ...
            <View style={{ transform: transform(), flex: 1, backgroundColor: 'white' }}>
                ...
                <View style={{ width: w, height: width / 6.5, backgroundColor: 'white', flexDirection: 'row', borderBottomWidth: 1, borderBottomColor: '#ddd' }}>
                    <TouchableOpacity onPress={() => this.props.navigation.goBack()}>
                        ...
                        <Text style={{ marginLeft: width / 7, bottom: width / 30, fontSize: width / 18, color: '#333', transform: transform(), textAlign: textAlign(), fontWeight: 'bold', }}>{i18n.t('addWorkout')}</Text>
                    </TouchableOpacity>
                </View>
                <ScrollView contentContainerStyle={{ paddingBottom: width / 20 }}>
                    ...
                    <Text style={{ marginTop: width / 20, marginLeft: width / 30, fontSize: width / 22, fontWeight: 'bold', color: '#333', transform: transform(), textAlign: textAlign(), }}>{i18n.t('assignWorkout')}</Text>
...
export default AddMemberWorkout