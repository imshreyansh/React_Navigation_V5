...
class TrainerQuestionOne extends Component {

    state = {
        level: [{ name: i18n.t('beginner'), isChecked: false }, { name: i18n.t('intermidiate'), isChecked: false }, { name: i18n.t('advanced'), isChecked: false }],
        rtl: null
    }
    ...
    onSubmit = () => {
        ...
        const id = {
            ...
            trainer: this.props.navigation.getParam('id').trainerId,
            package: this.props.navigation.getParam('id').packageId,
            trainerAmount: this.props.navigation.getParam('id').trainerFees,
            questionOne: data.length === 0 ? '' : data[0].name,
        ...
        if (id.questionOne !== '') {
            this.props.navigation.navigate('TrainerQuestionTwo', { id, oldPackageId: this.props.navigation.getParam('oldPackageId') })
        } else {
                ...
                <ScrollView contentContainerStyle={{ paddingBottom: width / 10 }}>
                    ...
                    <View style={{ marginLeft: 'auto', marginRight: 'auto', width: w / 1.1, backgroundColor: 'white', marginTop: width / 15, paddingBottom: width / 10, borderRadius: 3 }}>
                        <View style={{ alignItems: 'center', marginTop: width / 20 }}>
                            <Text style={{ fontSize: width / 20, fontWeight: 'bold', color: 'grey', transform: transform(), textAlign: textAlign(), textAlign: 'center' }}>{i18n.t('whatIsYourLevel')}</Text>
                            <Text style={{ fontSize: width / 30, marginTop: width / 80, transform: transform(), textAlign: textAlign(), color: 'grey' }}>{i18n.t('selectYourFitnessLevel')}</Text>
                        </View>
...
export default TrainerQuestionOne