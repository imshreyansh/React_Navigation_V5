...
class PackagesInvoice extends Component {
    _isMounted = false
    ...
    onViewInvoice = (data) => {
        if (data.paidStatus === 'Installment') {
            ...
            let remainAmount = parseFloat(data.packages.amount) - parseFloat(toBePaid)
            ...
            this.setState({
                ...
                totalAmount: data.packages.amount
            }, () => {
                this.setModalVisible(true)
            })
        } else {
            this.setModalVisible(false)
            this.props.navigation.navigate('InvoiceDetails', { orderDetails: data, packageDetails: data, type: 'packages' })
        }
    ...
    navigateByInstallment = (data) => {
        this.setModalVisible(false)
        this.props.navigation.navigate('InvoiceDetails', { orderDetails: data, packageDetails: this.state.selectedOrder, type: 'packages' })
    }
                        ...
                        return (
                            <View key={i} style={{ width: w / 1.1, borderWidth: 1, backgroundColor: 'white', borderColor: '#ddd', paddingBottom: width / 50, marginLeft: 'auto', marginRight: 'auto', marginTop: width / 20, transform: transform() }}>
                                <View style={{ flexDirection: 'row', justifyContent: 'space-between', width: w / 1.15, marginTop: width / 60, marginLeft: 'auto', marginRight: 'auto' }}>
                                    <View>
                                        <Text numberOfLines={1} style={{ fontSize: width / 25, color: '#333', width: w / 2.2, fontWeight: 'bold', transform: transform(), textAlign: textAlign() }}>{d.packages.packageName}</Text>
                                    </View>
                                    <View style={{ flexDirection: 'row' }}>
                                        <Text numberOfLines={1} style={{ fontSize: width / 28, color: '#333', width: w / 4, top: width / 90, transform: transform(), textAlign: textAlign() }}>{new Date(d.startDate).getDate()}/{new Date(d.startDate).getMonth() + 1}/{new Date(d.startDate).getFullYear()}</Text>
                                        {d.paidStatus === 'Installment' || d.paidStatus === 'Paid' ?
                                            <TouchableOpacity onPress={() => this.onViewInvoice(d)}>
                                                <Icon name="eye" size={width / 15} style={{ marginLeft: width / 60 }} color="#1976d2" />
                                            </TouchableOpacity> : <Icon name="eye" size={width / 15} style={{ marginLeft: width / 60 }} color="white" />}
                                    </View>
                                </View>
                            </View>
                        )
                            ...
                            <ScrollView contentContainerStyle={{ paddingBottom: width / 30 }}>
                                <View style={{ marginTop: width / 30, marginLeft: 'auto', marginRight: 'auto' }}>
                                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', width: width - 50, }}>
                                        <Text style={{ fontSize: width / 30, color: '#333', fontWeight: 'bold', width: w / 4.5, textAlign: 'center' }}>{i18n.t('packageName')}</Text>
                                ...
                                <View style={{ marginTop: width / 30, marginLeft: width / 30 }}>
                                    <Text style={{ fontSize: width / 25, color: '#333', fontWeight: 'bold' }}>Installment History</Text>
                                </View>
...
export default PackagesInvoice