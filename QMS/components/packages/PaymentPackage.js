...
class PaymentPackage extends Component {
    ...
    componentDidMount() {
        ...
        if (this._isMounted) {
            ...
            this.setState({
                ...
                getPackagesByID(this.state.data.package).then(res => {
                    if (res) {
                        this.setState({
        ...
        if (this.props.navigation.getParam('oldPackageId') !== undefined) {
            const data = {
                questions: {
                    ...
                    goalQuestion: this.props.navigation.getParam('id').questionThree
                }
            }
            const datas = {
                totalAmount: (this.props.navigation.getParam('id').trainerAmount + this.state.packageDetails.amount + (this.state.packageDetails.amount * (this.state.tax / 100))).toFixed(2),
            ...
            payOnline(datas).then(res => {
                if (res) {
                    ...
                    this.setState({ loading: false }, () => {
                        updateMember(this.props.navigation.getParam('id').id, data).then(res => {
                            if (res) {
                                this.setState({
                                    loading: false,

                                })
                                this.props.navigation.navigate(this.state.navigation)
                            }
                        })
                    })
                }
    ...
    onPayAtGymRenew = () => {
        const id = this.props.navigation.getParam('id')
        const totalAmount = (id.trainerAmount + this.state.packageDetails.amount + (this.state.packageDetails.amount * (this.state.tax / 100))).toFixed(2)
        if (this.props.navigation.getParam('oldPackageId') !== undefined) {
            const data = {
                questions: {
                    levelQuestion: id.questionOne,
                    exercisingQuestion: id.questionTwo,
                    goalQuestion: id.questionThree
                },
                packageDetails: [{
                    packages: id.package,
                    trainer: id.trainer,
    ...
    renderButtonsPay = () => {
        if (this.state.gym === true) {
            return (
                <TouchableOpacity onPress={() => { this.props.navigation.getParam('id').didFingerAuth === true ? this.onPayAtGymRenew() : this.onPayAtGym() }}>
    ...
    render() {
        const totalAmount = (this.props.navigation.getParam('id').trainerAmount + this.state.packageDetails.amount + (this.state.packageDetails.amount * (this.state.tax / 100))).toFixed(3)
        return (
                ...
                >
                    <View style={{ backgroundColor: '#f3e5f5', height: width / 1.8, width: width - 40, marginLeft: 'auto', marginRight: 'auto', marginTop: 'auto', marginBottom: 'auto', borderRadius: 5, transform: [{ rotateY: this.state.isRTL ? '180deg' : '0deg' }] }}>
                        ...
                        <View style={{ marginTop: width / 20, marginLeft: width / 30 }}>
                            <Text style={{ fontSize: width / 22, color: '#7e57c2' }}>{this.state.doneFingerAuth === true ? 'You have successfully requested a package for yourself' : 'You have successfully requested a package for yourself, now proceed to gym for further process !'}</Text>
                        </View>
